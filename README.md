...
## 목차
  ...
  * [8. GraphQL에 대한 개인적인 견해](#8-graphql에-대한-개인적인-견해)
    + [8-1.  GraphQL 사용 시 파일 업로드 작업이 불편하다.](#8-1-graphql-사용-시-파일-업로드-작업이-불편하다)
    + [8-2. 문서화의 불편함](#8-2-문서화의-불편함)
    + [8-3. 테스트 코드 작성 시 실수하기 쉬움](#8-3-테스트-코드-작성-시-실수하기-쉬움)
    + [8-4. 클라이언트가 자유롭게 쿼리를 호출함으로써 발생하는 부작용](#8-4-클라이언트가-자유롭게-쿼리를-호출함으로써-발생하는-부작용)
    + [8-5. 마이크로 서비스(MSA)에서의 복잡성 증가](#8-5-마이크로-서비스msa에서의-복잡성-증가)
  * [9. 참고자료](#9-참고자료)
...
### 8-1.  GraphQL 사용 시 파일 업로드 작업이 불편하다.
...
### 8-2. 문서화의 불편함
  - 비즈니스 로직을 작성하고나서, `schema.graphqls` 파일에 반드시 타입과 쿼리를 작성해야 한다.
    - 이때 작성한 내용이 리졸버의 메서드명 다를 경우 호출되지 않는다.
      - → 직접 타이핑을 하는 방식이기 때문에, 수작업이 들어가 실수를 불러일으키기 쉽다.
...
### 8-3. 테스트 코드 작성 시 실수하기 쉬움
  - query를 하드코딩하는 과정에서 실수를 불러일으키기 쉽다.
  - 또한 스펙이 바뀌었을 경우, 테스트 코드로 돌아와서 일일이 쿼리를 수정하는 수작업이 들어간다.
...
### 8-4. 클라이언트가 자유롭게 쿼리를 호출함으로써 발생하는 부작용
  - 클라이언트가 요청을 불필요하게 무거운 쿼리를 보냄으로써 서버에 부담이 가해질 수 있다.

### 8-5. 마이크로 서비스(MSA)에서의 복잡성 증가
  - 백엔드 서버가 MSA인 경우 클라이언트에서 각 서비스에 API 요청을 각각 보내야 해서 또다시 언더페칭이 발생한다.
  - 언더페칭을 해결하기 위해 복잡성을 감수하며 GraphQL을 채택했는데, 다시 언더페칭이 발생한다면 결국 복잡하다는 단점만 남게 되는 것은 아닐까?
